...
let bufferedRecords: TrainingRecord[] = [];
let shardIndex = 0;
...
async function flushShard(): Promise<void> {
  if (bufferedRecords.length === 0) return;
  const file = `positions-${String(shardIndex).padStart(5, "0")}.jsonl.gz`;
  const contents = `${bufferedRecords.map((record) => JSON.stringify(record)).join("\n")}\n`;
  const compressed = gzipSync(Buffer.from(contents), { level: 6 });
  await writeFile(resolve(options.output, file), compressed);
  shards.push({ file, records: bufferedRecords.length, bytes: compressed.byteLength });
  bufferedRecords = [];
  shardIndex += 1;
...
await writeFile(resolve(options.output, "schema.json"), `${JSON.stringify({
  ...
  caveat: "On-policy Monte Carlo labels generated by the recorded beam-search depth and width",
}, null, 2)}\n`);
await writeManifest("running");
...
for (let gameIndex = 0; gameIndex < options.maxGames; gameIndex += 1) {
  ...
  if (bufferedRecords.length >= options.shardRecords) await flushShard();
  else await writeManifest("running");
